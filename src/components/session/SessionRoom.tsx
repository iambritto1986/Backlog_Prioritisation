...
export const SessionRoom: React.FC<SessionRoomProps> = ({
  ...
  // Live knock and authorization state
  const [pendingKnocks, setPendingKnocks] = useState<{guestId: string, guestName: string}[]>([]);
  ...
  // Live Authentication Layer: Facilitator socket connection
  useEffect(() => {
    ...
    s.on('connect', () => {
      s.emit('host_session', { sessionId: session.id, facilitatorName: currentUser.name });
    });

    s.on('guest_knock', (data: { guestId: string, guestName: string, sessionId: string }) => {
      if (data.sessionId === session.id) {
        setPendingKnocks((prev) => [...prev, { guestId: data.guestId, guestName: data.guestName }]);
      }
    });
  ...
  }, [isFacilitator, session.id, currentUser.name]);

  const handleApproveGuest = (guestId: string) => {
    socket?.emit('approve_guest', { sessionId: session.id, guestId });
    setPendingKnocks((prev) => prev.filter((k) => k.guestId !== guestId));
  };

  const handleRejectGuest = (guestId: string) => {
    socket?.emit('reject_guest', { sessionId: session.id, guestId });
    setPendingKnocks((prev) => prev.filter((k) => k.guestId !== guestId));
  };
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-3.5rem)] overflow-hidden bg-[#faf9f5] dark:bg-[#18191c] text-stone-900 dark:text-stone-100">
      ...
      {isFacilitator && pendingKnocks.length > 0 && (
        <div className="fixed bottom-6 right-6 z-50 w-80 bg-[#1C1C1C] border-2 border-[#d4af37]/50 rounded-xl shadow-2xl p-4 animate-in slide-in-from-bottom-8">
          <div className="flex items-center gap-2 mb-3 text-[#d4af37]">
            <Lock className="w-5 h-5" />
            <h3 className="font-bold">Entry Requests ({pendingKnocks.length})</h3>
          </div>
          <div className="space-y-3 max-h-[300px] overflow-y-auto">
            {pendingKnocks.map((k) => (
              <div key={k.guestId} className="p-3 bg-[#111111] rounded-lg border border-[#333333] flex flex-col gap-2">
                <span className="text-[#EAEAEA] font-medium text-sm">{k.guestName}</span>
                <div className="flex gap-2">
                  <button
                    onClick={() => handleRejectGuest(k.guestId)}
                    className="flex-1 py-1.5 text-xs font-semibold text-stone-400 hover:text-white border border-stone-700 hover:border-red-500 rounded bg-stone-800 transition-colors"
                  >
                    Deny
                  </button>
                  <button
                    onClick={() => handleApproveGuest(k.guestId)}
                    className="flex-1 py-1.5 text-xs font-semibold text-black bg-[#d4af37] hover:bg-[#c59e2b] rounded transition-colors"
                  >
                    Approve
                  </button>
                </div>
